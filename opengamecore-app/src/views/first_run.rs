...
pub fn view(phase: &FirstRunPhase) -> Element<'_, Message> {
    let content: Element<'_, Message> = match phase {
        FirstRunPhase::Welcome => {
            ...
            let skip = button(text("Skip for now").size(13).color(theme::TEXT_SECONDARY))
        ...
        FirstRunPhase::Downloading { progress, status } => {
            ...
            let bar = progress_bar(0.0..=100.0, *progress).width(400).height(8);

            let status_text = text(status).size(13).color(theme::TEXT_SECONDARY);
        ...
        FirstRunPhase::Done => {
            let title = text("All set!").size(28).color(theme::ACCENT);
        ...
        FirstRunPhase::GamesFound { detected } => {
            let title = text("Games Found!").size(28).color(theme::ACCENT);
                ...
                let game_row = container(
                    row![
                        text(&game.name)
                            ...
                            .color(theme::TEXT_PRIMARY)
                            .width(Length::Fill),
                        text(rating_text).size(12).color(theme::TEXT_SECONDARY),
                        {
                            let action_element: Element<'_, Message> = if game.bundle_available {
                                button(text("Set Up").size(12).color(theme::BUTTON_GREEN_TEXT))
